...
describe('Phase 5 구조 가드 — SolutionSection + FeaturesSection 추가', () => {
  ...
  it('FeaturesSection scope 내부에 정확히 16개 article + 16개 status badge 가 존재한다 (Phase 11 v2)', () => {
    const { container } = render(<App />);
    const features = container.querySelector('[data-testid="features-section"]');
  ...
  it('SolutionSection scope 내부에 정확히 4개 article 이 존재한다 (Phase 11 v2: memory 추가)', () => {
    const { container } = render(<App />);
    const solution = container.querySelector('[data-testid="solution-section"]');
...
describe('Phase 6 구조 가드 — ScenariosSection + DifferentiationSection 추가', () => {
  ...
  it('ScenariosSection scope 내부에 정확히 6개 article 이 존재한다 (Phase 11 v2: 4 → 6 시나리오)', () => {
    const { container } = render(<App />);
    const scenarios = container.querySelector('[data-testid="scenarios-section"]');
  ...
  it('DifferentiationSection scope 내부에 정확히 5개 article 이 존재한다 (Phase 11 v2: 3 → 5 비교축)', () => {
    const { container } = render(<App />);
    const diff = container.querySelector('[data-testid="differentiation-section"]');
  ...
  // Phase 6 렌더 순서 가드: Phase 7 구조 가드(TEST-P7.11) 가 전체 순서를 포함하므로
  // P6 전용 strict 가드는 제거. P6 섹션의 data-testid 존재 가드는 위에서 개별 검증.
});

// ─────────────────────────────────────────────────────────────
// Phase 7 구조 가드 (TEST-P7.10 + P7.11)
// ─────────────────────────────────────────────────────────────
describe('Phase 7 구조 가드 — AIModesSection + SafetySection 추가', () => {
  it('AIModesSection 이 data-testid="aimodes-section" 으로 렌더된다 (TEST-P7.10)', () => {
    const { container } = render(<App />);
    const el = container.querySelector('[data-testid="aimodes-section"]');
    expect(el, 'AIModesSection data-testid 없음').not.toBeNull();
    expect(el?.tagName).toBe('SECTION');
  });

  it('SafetySection 이 data-testid="safety-section" 으로 렌더된다 (TEST-P7.10)', () => {
    const { container } = render(<App />);
    const el = container.querySelector('[data-testid="safety-section"]');
    expect(el, 'SafetySection data-testid 없음').not.toBeNull();
    expect(el?.tagName).toBe('SECTION');
  });

  it('섹션 렌더 순서가 Phase 7 설계와 일치한다 (TEST-P7.11)', () => {
    // 리뷰 피드백 반영 (Medium): 이전 버전은 data-testid 만 수집해서
    // 데모 #roadmap 위치를 검증하지 못했다. data-testid 또는 id 가 있는
    // 모든 section 을 DOM 순서로 수집해 전체 페이지 흐름을 완전히 고정.
    const { container } = render(<App />);
    ...
    expect(allSections).toEqual([
      ...
      'problem-section',
      'solution-section',
      // features-section 은 id="features" + data-testid 양쪽 보유
      // data-testid 를 우선 사용하므로 'features-section' 으로 출력
      'features-section',
      'scenarios-section',
