...
- [x] **[REGRESSION-BASELINE]** Phase 6 진입 전 기준선 확보
  ```bash
  cd extapp_landing
  npm test
...
- [x] **[CONTEXT]** 작업 목적 확인
  ...
  - 시나리오 4개 *(사후 순서 재배치 반영 — §7.2 참조)*:
    | # | 시나리오 | i18n key |
    |---|---------|----------|
    | 1 | 사이트별 맞춤 스크립트로 확장 | `s1` |
    | 2 | 기사 문장 선택 → 번역·질문 | `s2` |
    | 3 | 그룹웨어 문서 작성 보조 | `s3` |
    | 4 | 탭 정리·페이지 탐색 자동화 | `s4` |
  - 차별화 비교 3쌍:
...
- [x] **[CONTEXT]** Phase 6 가 수정하는 파일과 Phase 1~5 가드의 상호작용
  | 수정 대상 | 영향 받는 가드 | 주의 |
  |----------|---------------|------|
...
- [x] **[RED]** DifferentiationSection 테스트
...
- [x] **[RED]** App.test.tsx Phase 6 구조 가드
...
- [x] **[TASK-002]** ScenariosSection 컴포넌트
...
- [x] **[TASK-003]** DifferentiationSection 컴포넌트
...
- [x] **[TASK-004]** App.tsx 갱신 — Scenarios/Diff 삽입 + 데모 2개 제거
...
- [x] **[REFACTOR-STRUCTURE]** 시나리오/비교 카드 데이터화 확인
- [x] **[REFACTOR-STRUCTURE]** 화살표 아이콘 일관성 (lucide `ArrowRight`)
- [x] **[REFACTOR-STRUCTURE]** App.tsx 에서 Button import 제거 가능 여부 확인
...
- [x] **[VERIFY]** 기능/시각 회귀 확인
- [x] **[DOC]** 작업 결과서 작성 — `working_history/v1.0/Phase6_ScenariosDiff_2026MMDD.md`
- [ ] **[COMMIT]** 커밋
...
- [x] ScenariosSection 4개 카드 + id="scenarios" (TEST-P6.1/P6.3)
- [x] 각 시나리오 카드에 step + h3 + 설명 (TEST-P6.2)
- [x] DifferentiationSection 3개 비교 카드 + id="differentiation" (TEST-P6.4/P6.6)
- [x] 각 비교 카드에 before/after 두 영역 (TEST-P6.5)
- [x] scenarios.*, differentiation.* 키 ko/en 동기화 (TEST-P6.7)
- [x] 언어 전환 정상 (TEST-P6.8)
- [x] data-testid="scenarios-section" / "differentiation-section" 공개 계약 (TEST-P6.9)
- [x] 섹션 렌더 순서 정확 (TEST-P6.10)
- [x] 4개 시나리오 + 3개 비교 카드 정체성 i18n 고정 (TEST-P6.11/P6.12)
- [x] `npm run lint` / `typecheck` / `format:check` / `test` / `build` 전부 통과
- [x] Phase 1~5 회귀 가드 유지 (NAV_ANCHORS, h1 유일성, data-testid 4종, FeaturesSection badge)
- [x] 데모 scenarios/differentiation 삭제 + roadmap 1개 잔존
- [x] App.test.tsx 데모 전용 테스트 6건 삭제 (Button 4건 + Anchor Link + Design System Demo h2)
- [ ] 작업 결과서 작성 및 커밋 완료
