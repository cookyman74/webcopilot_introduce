...
| 섹션 범위 | 기획서 **11개 섹션** 전부 1차 구현 (v2에서 BusinessSection 추가) |
...
└── extapp_landing/                   ← 실제 프로젝트 루트
    ...
    │   │       ├── BusinessSection.tsx          ← 5.10 (v2 추가 — B2B 대상)
    │   │       └── FinalCTASection.tsx          ← 5.11 (구 5.10)
    │   └── lib/
    │       ├── constants.ts          ← CTA URL, PARTNERSHIP_CONTACT 등
    │       └── types.ts
    ├── index.html
    ├── package.json
    ├── tsconfig.json
...
기획서 **11개 섹션**을 컴포넌트 단위로 1차 구현한다 *(v2: BusinessSection 추가)*.
...
### 5.10 BusinessSection *(v2 신규 — B2B 대상)*

> **대응 기획**: `01_landing_page_plan.md` §5.10
> **대상**: §2.2 #5 (해당 확장앱 또는 확장앱에 사용된 기술을 이용하여 사업에 적용하고 싶은 기업)
> **톤**: 제품 홍보가 아니라 **기술 재사용 제안**. 일반 카피와 시각적으로도 구분되어야 함.

- **배경**: `bg-surface-alt` (차별화 섹션과 같은 톤으로 시각 구분. `bg-accent-soft`는 §5.11 FinalCTA에 예약되어 있으므로 충돌 금지)
- **id**: `business` (Header 네비에는 포함하지 않음 — 전환 혼선 방지)
- **레이아웃**: 상단 카피 블록 + 하단 3카드 그리드 + 중앙 정렬 B2B CTA
- **요소**
  - Eyebrow 라벨: "For Businesses" / "기술 파트너십"
  - H2: "이 확장앱의 기술을 자사 서비스에 적용할 수 있습니다" *(예시)*
  - Sub: 2~3줄 — 페이지 문맥 AI · Action Tools · 스크립트 인프라가 재사용 가능한 기반임을 설명
  - **가치 카드 3개** (`FeatureCard` 재사용 — Badge **미사용**):
    1. 페이지 문맥 기반 AI — "사용자 컨텍스트를 잃지 않는 AI 파이프라인"
    2. Action Tools 에이전트 — "브라우저를 직접 조작하는 안전 경계 기반 자동화"
    3. 스크립트 실행/등록 인프라 — "사이트별 확장 자산을 안전하게 운영"
  - **B2B CTA 2개**
    - Primary: "파트너십 문의" → `mailto:${PARTNERSHIP_CONTACT}` (1차엔 mailto, `constants.ts`에 단일 출처)
    - Secondary: "기술 문서 보기" → `#` (1차 placeholder)
- **Badge 사용 금지**: 이 섹션의 카드는 "구현 상태 주장"이 아닌 "적용 가능성 제안"이므로 `구현됨/보강 중/계획·검토 중` 배지를 쓰지 않는다. 이를 테스트로 강제한다 (Phase 8 TEST-P8.X 참조).
- **1차 범위 제외**
  - 실제 문의 폼(POST 받는 엔드포인트) → 후속 이터레이션
  - 고객사 로고/레퍼런스 → 확정 전까지 미표기
  - B2B용 Header 네비 링크 → 추가하지 않음 (전환 메시지 분산 방지)

### 5.11 FinalCTASection *(구 5.10)*
...
- **주의**: 이 섹션은 **일반 사용자 대상 설치 유도**에만 집중한다. B2B 문의는 바로 위 §5.10 BusinessSection에서 처리되므로 여기서 파트너십/mailto를 중복 노출하지 않는다. 두 CTA가 섞이면 둘 다 약해진다.
...
// src/i18n/locales/ko.json
{
  ...
  "business": {
    ...
    "subtitle": "...",
    "cards": {
      "context":    { "title": "페이지 문맥 기반 AI",      "desc": "..." },
      "actionTools":{ "title": "Action Tools 에이전트",   "desc": "..." },
      "scripts":    { "title": "스크립트 실행/등록 인프라","desc": "..." }
    },
    "cta_primary": "파트너십 문의",
...
### Phase 3. 섹션 구현 (11개) *(v2: BusinessSection 추가)*
...
8. SafetySection
9. RoadmapSection
10. **BusinessSection** *(v2 신규 — B2B 대상)*
11. FinalCTASection
...
- [ ] **11개 섹션**이 순서대로 렌더링됨 *(v2)*
- [ ] 한국어/영어 전환이 동작함
- [ ] 주 CTA가 Chrome Web Store로 이동함
- [ ] **BusinessSection의 Primary CTA가 `mailto:${PARTNERSHIP_CONTACT}` 로 열림**
- [ ] **BusinessSection 카드에 상태 배지가 렌더되지 않음** (제품 기능 섹션과의 혼동 방지)
- [ ] 데스크톱/태블릿/모바일 3개 브레이크포인트에서 깨짐 없음
- [ ] 상태 배지(`구현됨`/`보강 중`/`계획/검토 중`)가 기획서 규칙대로 적용됨 *(BusinessSection 제외)*
